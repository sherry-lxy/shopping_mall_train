...
from model.whitening import PCAWhitening
...
PATH = "/home/li/main/shopping_mall_train/lightning_logs/version_26/checkpoints/epoch=149-step=5250.ckpt"
net = model.load_from_checkpoint(PATH)
net.freeze()
...
for i, data in tenumerate(data_query):
    ...
    pred = net(data).to("cpu").detach().numpy().copy().squeeze()
...
for i in tqdm(range(n_query)):
    sim = np.dot(query_pred[i], reference_pred.T) / (
        np.linalg.norm(query_pred[i]) * np.linalg.norm(reference_pred, axis=1)
    )
